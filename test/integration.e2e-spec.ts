...
describe('Integration Tests - Both Auth Types (e2e)', () => {
  ...
  describe('Bot Creates User → User Can Access via JWT', () => {
    it('should allow bot to create user and user to access via JWT', async () => {
      ...
      const createResponse = await testHelpers.makeBotRequest(
        'POST',
        '/internal/users',
        validUserCreateDto,
      );
    ...
    it('should allow bot to update user and user to see updated data', async () => {
      ...
      await testHelpers.makeBotRequest(
        'POST',
        '/internal/users',
        validUserCreateDto,
      );
      ...
      const updateResponse = await testHelpers.makeBotRequest(
        'PATCH',
        `/internal/users/${validUserCreateDto.id}`,
        updateData,
      );
  ...
  describe('User Updates Data → Bot Can Fetch It', () => {
    it('should allow user to update settings and bot to fetch updated data', async () => {
      ...
      const botFetchResponse = await testHelpers.makeBotRequest(
        'GET',
        `/internal/users/${validUserCreateDto.id}`,
      );
    ...
    it('should allow user to update profile settings and bot to see changes', async () => {
      ...
      const botResponse = await testHelpers.makeBotRequest(
        'GET',
        `/internal/users/${validUserCreateDto.id}`,
      );
  ...
  describe('Cross-Auth-Type Data Flow', () => {
    it('should maintain data consistency between bot and user operations', async () => {
      ...
      const botUserResponse = await testHelpers.makeBotRequest(
        'GET',
      ...
      expect(userProfileResponse.body.username).toBe(
        botUserResponse.body.username,
      );
    });
  ...
  describe('Authorization Boundaries', () => {
    it('should prevent user from accessing other users data', async () => {
      ...
      expect(unauthorizedResponse.body.message).toContain(
    ...
    it('should allow bot to access all user data', async () => {
      ...
      const allUsersResponse = await testHelpers.makeBotRequest(
        'GET',
        '/internal/users',
      );
      ...
      const specificUserResponse = await testHelpers.makeBotRequest(
        'GET',
        '/internal/users/222222222222222222',
      );
  ...
  describe('Error Handling Across Auth Types', () => {
    it('should handle bot operations on non-existent user', async () => {
      const response = await testHelpers.makeBotRequest(
        'GET',
        '/internal/users/nonexistent',
      );
    ...
    it('should handle unauthenticated requests appropriately', async () => {
      ...
      const botResponse = await testHelpers.makeUnauthenticatedRequest(
        'GET',
        '/internal/users',
      );
      ...
      const userResponse = await testHelpers.makeUnauthenticatedRequest(
        'GET',
        '/api/profile',
      );
