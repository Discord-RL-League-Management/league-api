...
export class UserGuildsService {
   ...
   */
  async getUserAvailableGuildsWithPermissions(userId: string): Promise<UserGuild[]> {
    try {
   ...
   */
  async completeOAuthFlow(userId: string, userGuilds: DiscordGuild[]): Promise<UserGuild[]> {
    try {
      // Sync guild memberships atomically
      await this.syncUserGuildMemberships(userId, userGuilds);

      // Get enriched guild data
      const availableGuilds = await this.getUserAvailableGuildsWithPermissions(userId);
