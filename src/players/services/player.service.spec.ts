...
import { PlayerNotFoundException, PlayerAlreadyExistsException } from '../exceptions/player.exceptions';
...
describe('PlayerService', () => {
  ...
  beforeEach(async () => {
    ...
    validationService = module.get<PlayerValidationService>(PlayerValidationService);
  ...
  describe('findOne', () => {
    it('should return a player when found', async () => {
      const mockPlayer = { id: 'player1', userId: 'user1', guildId: 'guild1', status: 'ACTIVE' };
      mockPlayerRepository.findById.mockResolvedValue(mockPlayer);

      const result = await service.findOne('player1');

      expect(result).toEqual(mockPlayer);
      expect(mockPlayerRepository.findById).toHaveBeenCalledWith('player1', undefined);
    });

    it('should throw PlayerNotFoundException when player not found', async () => {
      mockPlayerRepository.findById.mockResolvedValue(null);

      await expect(service.findOne('player1')).rejects.toThrow(PlayerNotFoundException);
    });
  });
  ...
  describe('create', () => {
    ...
    it('should create a player successfully', async () => {
      mockValidationService.validateGuildMembership.mockResolvedValue(undefined);
      mockPlayerRepository.findByUserIdAndGuildId.mockResolvedValue(null);
      mockPrismaService.$transaction.mockImplementation(async (callback) => {
        const tx = {
          player: {
            create: jest.fn().mockResolvedValue({ id: 'player1', ...createDto }),
          },
        };
        return callback(tx);
      });
      ...
      expect(mockValidationService.validateGuildMembership).toHaveBeenCalledWith('user1', 'guild1');
      expect(mockPlayerRepository.findByUserIdAndGuildId).toHaveBeenCalledWith('user1', 'guild1');
      expect(result).toHaveProperty('id');
    });

    it('should throw PlayerAlreadyExistsException when player already exists', async () => {
      mockValidationService.validateGuildMembership.mockResolvedValue(undefined);
      mockPlayerRepository.findByUserIdAndGuildId.mockResolvedValue({ id: 'existing' });

      await expect(service.create(createDto)).rejects.toThrow(PlayerAlreadyExistsException);
    });
  });

  describe('ensurePlayerExists', () => {
    it('should return existing player if found', async () => {
      const existingPlayer = { id: 'player1', userId: 'user1', guildId: 'guild1' };
      mockPlayerRepository.findByUserIdAndGuildId.mockResolvedValue(existingPlayer);
      ...
      mockPrismaService.$transaction.mockImplementation(async (callback) => {
        const tx = {
          player: {
            ...
            create: jest.fn().mockResolvedValue({ id: 'player1', userId: 'user1', guildId: 'guild1' }),
          },
        };
        return callback(tx);
      });
