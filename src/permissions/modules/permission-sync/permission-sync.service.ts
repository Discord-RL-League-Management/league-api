...
export class PermissionSyncService {
  ...
  async syncUserPermissions(userId: string, guildId: string): Promise<void> {
    try {
      ...
      if (!accessToken) {
        this.logger.warn(
          `No valid access token for user ${userId}, skipping permission sync`,
        );
        return;
      }

      const discordPermissions =
        await this.discordValidation.checkGuildPermissions(
          accessToken,
          guildId,
        );
      ...
      if (!discordPermissions.isMember) {
        ...
        this.logger.log(
          `Removed user ${userId} from guild ${guildId} - no longer a member`,
        );
        return;
      }

      await this.guildMembersService.updateMemberRoles(
        userId,
        guildId,
      ...
      this.logger.log(
        `Synced permissions for user ${userId} in guild ${guildId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error syncing user permissions for user ${userId} in guild ${guildId}:`,
        error,
      );
      throw error;
    }
